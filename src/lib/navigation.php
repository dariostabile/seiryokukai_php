...
function map_application_to_page(string $urlApplicazione): ?string
{
    return match ($urlApplicazione) {
        ...
        'utenti' => 'users',
        'sedi' => 'sites',
        'tipi_documento' => 'document_types',
        'disciplina', 'discipline' => 'disciplines',
        'corsi' => 'courses',
        default => null,
    };
}

function navigation_menu_for_user(int $userId): array
{
    if ($userId <= 0) {
        return [];
    }
    ...
    foreach ($rows as $row) {
        ...
        $page = map_application_to_page($appUrl);
