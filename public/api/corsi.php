...
function eliminaImmagineCorsoDaPercorso(string $path): void
{
    ...
    if ($cleanPath === '' || strpos($cleanPath, 'public/corsi/') !== 0) {
        return;
    }
...
/**
 * Gestisce upload immagine corso da file (multipart/form-data)
 */
...
// ---------------------------------------------------------------------------
// Funzioni di supporto HTTP / risposta
// ---------------------------------------------------------------------------
    ...
    foreach ($post as $key => $value) {
        ...
        if (is_array($value)) {
            $serialized = array_map(
                static fn ($item): string => is_scalar($item) || $item === null ? (string) $item : '',
                $value
            );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update') {
        try {
            ...
            // Rimozione esplicita immagine
            if ($removeImage) {
                $newImagePath = '';
            }

            // Upload da crop base64 (ha precedenza sul file classico)
            ...
            // Upload file classico (solo se non c'è crop)
            $fileImmagine = $_FILES['immagine_corso'] ?? [];
            if (
                $croppedImageData === ''
                && !empty($fileImmagine)
                && ($fileImmagine['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE
            ) {
            ...
            // Persisti e rimuovi vecchio file solo se il path è cambiato
            ...
            if ($wantsJson) {
                json_success('Corso modificato con successo', ['id' => $corsoId]);
            }
    ...
    try {
        ...
        // Upload immagine (crop base64 ha precedenza)
        $croppedImageData = trim((string) ($_POST['crop_image_base64'] ?? ''));
        if ($croppedImageData !== '') {
            $imagePath = gestisciUploadImmagineBase64Corso($croppedImageData, $nuovoId);
            if ($imagePath !== '') {
        ...
        handle_validation_errors($e->errors(), 'corsi', $addContext);
    } catch (\InvalidArgumentException $e) {
        if ($wantsJson) {
            handle_validation_errors_json(['user_id' => $e->getMessage()], 422);
        }
        $addContext = build_add_context_from_post($_POST);
        handle_validation_errors(['user_id' => $e->getMessage()], 'corsi', $addContext);
    }
}
